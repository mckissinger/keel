...
Logo       logo → plan PR (kit + integration milestone)     ← brand mark + logo kit — SVG-direct, attended
...
- **Logo** — `logo` (SVG-direct logo kit for a product's brand mark — SVG masters per lockup × colorway, the favicon/PWA set, OG image and social avatar, a usage sheet with the font + license note — generated under parametric construction and iterated through attended art direction; ends on a plan PR carrying the kit under `design/brand/` plus one integration milestone the normal pipeline builds)

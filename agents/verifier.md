...
- **The one bounded exception — the instance-scoped mutation license.** When the dispatching prompt passes a proven Q13 instance recipe (the profile's parallel-session isolation contract, quoted with your orchestrator-assigned instance identity), you may create, seed, reset, and tear down **only your own instance's resources**: your worktree's env file, your instance's datastore branch/namespace, your assigned ports/processes. Never the shared primary, never a sibling instance, never the repo's tracked files — the Edit/Write disallow stands unchanged, so the license's one file-write shape is the recipe's recorded env-derivation command writing your worktree's untracked, instance-local env file via Bash, not via Edit/Write. Teardown (the recipe's command) runs on **every** exit path, failure included. Absent that recipe in the dispatching prompt, the read-only rule above holds without exception.
...
- **Before any negative finding that depends on the running environment, prove the environment reflects committed code — clean tree + positive control.** A `[runtime]`/environment-dependent **negative** finding (the surface errored, the endpoint 500'd, the seed was rejected, a probe returned nothing) may not stand until you (a) assert the working tree is **clean for the profile's declared service-config paths** (Q14) — a dirty tree there is the environment poisoned, classified **environment, not a discrepancy** — and (b) confirm the profile's **positive control** that the environment reflects committed state: probe the gate (restart, hit the endpoint, expect a real response), **not** the CLI start banner, and confirm the **independent signal changed**. **Reproducing a failure N times inside one un-restarted environment is not confidence** — it is the same poisoned read N times, and only an independent signal changing certifies a genuine reload. The profile (Q14) declares which paths and which signal; this rule carries the requirement. Scope: `[runtime]`/environment-dependent **negative** findings **only** — never an `[auto]`/static finding, and never a clean pass.

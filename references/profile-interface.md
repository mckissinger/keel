...
> ⚠ **Plan-only PRs never pay suite-grade CI.** A PR whose diff touches only plan paths cannot affect the suites, so suite/build-grade CI jobs **early-exit on plan-only diffs via the project's copied canonical classifier** (`scripts/check-verified-pin.sh --plan-only-check` — fail-closed: a classifier error runs everything, never skips), **never a workflow `paths:` filter on a required check** — a required check that never reports wedges the merge behind branch protection, while an early-exited job runs and reports. The classifier needs the merge base in the checkout (`fetch-depth: 0` or an equivalent deepened fetch). The pin gate, the plan lint, and the security review run on **every** PR — they are never fast-pathed.

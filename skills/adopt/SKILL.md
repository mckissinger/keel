---
name: adopt
description: Brownfield kickoff — stand up keel's durable foundation on an EXISTING codebase. A one-time attended retrofit that discovers the foundation from the code (stack, data model, design language, seams, invariants), derives the stack profile, seeds thin foundation specs, and wires the kickoff-tier CI gates (verified-pin + plan-lint) + branch protection once — then hands off to the per-feature loop.
...
5. **Process + CI.** Reuse the repo's existing process. Bootstrap only what the rules depend on **and only if absent**: copy keel's shipped `${CLAUDE_PLUGIN_ROOT}/scripts/check-verified-pin.sh` **and** `${CLAUDE_PLUGIN_ROOT}/scripts/check-plan.sh` and wire **both** as CI jobs — the `verified-pin` gate and the `plan-lint` gate, the same kickoff tier `spec-foundation` wires, so a retrofitted repo isn't missing a gate the rules assume (and copy `${CLAUDE_PLUGIN_ROOT}/scripts/repin.sh` alongside them — the canonical re-pin mechanizer the rebase re-pin rule invokes, copied not wired; copy them, don't re-author them — and write the `scripts/KEEL-SYNC` sync manifest in the same commit: first line `keel <installed plugin version>`, then one copied-script filename per line, updated on every later copy/re-sync so script drift is remedied as one batch sync chore PR, never serial per-script discovery), branch protection (required status checks with **require-branches-up-to-date** — a moved base must re-enter the drift window before merge), and file-per-entry `decisions/` + `deferrals/`. **Two jobs is the whole kickoff tier — the third required check the auto-preflight asserts (`security-review`) belongs to the autonomy tier and is wired at auto-entry at the latest** (genesis wires it at bootstrap; `auto:feature`/`auto:run` on a standing project wire it as preflight remediation; wiring it at kickoff is also valid, adding its context to the required set the moment the job exists, never before), per `${CLAUDE_PLUGIN_ROOT}/references/template-contract.md` tier 1, which records its default implementation **and the proven concrete recipe** (named-job shapes, the SHA-pinned review action, and the two protection shapes — wire the kickoff-tier shape, whose required contexts are only the checks being wired here). The CI-gate bootstrap is **code**, so land it as a small **one-time setup PR first** (it would break a later plan PR's plan-only status). If you're inside an existing host app, you inherit all of this.

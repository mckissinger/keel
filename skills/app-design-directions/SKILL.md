...
description: Design-SYSTEM exploration workflow for application UI (dashboards, CRMs, SaaS products, internal tools, data-heavy apps). Use this skill ONCE per app to decide and build the design system, or for a redesign — whenever the user wants to design an app's interface, explore visual directions, pick a design language, build a design system, theme components, or says "make this look good," "design options," "mockups," "new UI," "redesign," "what should this look like." It learns the app, generates 3–5 distinct directions as side-by-side HTML mockups, helps the user pick one, then installs the winner as a REVIEWABLE COMPONENT GALLERY (a styleguide route of real themed primitives) every feature composes from. NOT for designing one feature's screens against an already-decided system — that's spec-feature.
---
...
```
Phase 0  DISCOVER   → learn the app, write a design brief, confirm with user
Phase 1  DIRECT     → propose 3–5 structurally distinct directions (token specs)
Phase 2  MOCK       → build one self-contained HTML mockup per direction (one screen) + compare gallery
Phase 3  SELECT     → user reviews side-by-side, picks (or hybridizes), decision recorded
Phase 3.5 SPREAD    → mock the chosen direction across the app's screen archetypes (cap ~5), redline
Phase 4  APPLY      → translate winning direction into the real codebase, verify directionally
```

Copy this checklist into your working notes and check items off as you go:

- [ ] Phase 0: codebase explored, design brief written, brief confirmed by user
- [ ] Phase 1: 3–5 directions specced, each passed the anti-slop check and the distinctness check
- [ ] Phase 2: mockups built with real domain data, compare.html gallery assembled, self-reviewed via screenshots if a browser is available
...
- [ ] Phase 3.5: chosen direction mocked across screen archetypes (one per archetype, cap ~5), screenshot-reviewed, redlined by user
- [ ] Phase 4: tokens + components applied to real stack, screens verified for directional fidelity
...
2. **Self-contained HTML files.** One file per direction (`design/mockups/direction-name.html`), inline CSS, Google Fonts via link tag, no build step — they must open by double-click. Static is fine; a couple of hover states bring it to life.
...
**This gate decides and builds the design SYSTEM — not any one feature's screens.** After it, individual features are designed *per feature* by the `spec-feature` skill, which mocks that feature's own screens by **composing from this system** (the gallery + `design.md` tokens). Do **not** re-run this skill per feature — the direction is locked; per-feature screen design that re-explores directions is the bug. The system is decided once here; the screens are designed many times, downstream, from it.

## Phase 4 — Apply: build the design system as a reviewable component gallery

This phase builds the **system**, not the whole app. Its deliverable is a *styleguide route of real, themed primitives* that every feature later composes from — design as **code**, not prose. (Wide per-screen application happens downstream, per feature, in `spec-feature` + the feature's build milestones — not here.)

1. **Tokens first.** Install the design tokens at the root (CSS variables / `@theme`). Never hardcode a hex value in a component that exists as a token.
2. **Build the gallery — the whole material palette, as real components, on a `/styleguide` route.** Themed primitives replacing native controls (`<select>` → themed Select), the chosen icon package, the chosen motion library, the chosen chart/data-viz library — each themed to the palette — **and the load-bearing states every feature needs: empty, loading, error, populated.** This route is the reviewable artifact: the user signs off on the *system* here, and `spec-feature` assembles feature screens from these exact primitives so features can't drift to defaults. Screenshot it, compare against the direction for **directional fidelity** (palette, type, density, architecture, signature element) — not pixel parity; the real stack will and should look more polished than the static HTML. Fix *directional* drift only.
3. **Retheme, don't fight, the component library.** With shadcn/ui, change the CSS variables and a small number of component files — that's the whole point of owning the components. Default shadcn styling is itself a recognized AI fingerprint, so the tokens must actually move away from stock.
4. **Verify each converted screen visually** (screenshot loop again) and against the quality floor in `references/product-ui-craft.md`: keyboard focus visible, WCAG AA contrast, empty/loading/error states styled, responsive behavior sane, reduced-motion respected.
...
| `references/stack-and-mockups.md` | Phase 2 and 4. Mockup file mechanics, Tailwind v4/shadcn token wiring, screenshot self-review setup. |

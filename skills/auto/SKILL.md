...
Before anything is written, establish where the project sits on the **grain ladder** from `specs/` + git state: what `specs/00-product.md`'s backlog lists, which features have deep specs (`specs/features/`), which milestones exist / are pinned / are merged (`specs/milestones/`, `_landed/`, open PRs, branch state). That canonical state sweep is owned by the `status` skill — run its derivation rather than an ad-hoc one. From that position, write down **the run's definition of done for the given scope** — the concrete list of features/milestones this run will take from their current state to landed-and-debrief-pending.

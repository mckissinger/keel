...
- **Mode-gated.** Consults *under this contract* exist **only under an active mode** — a valid mode file per the step-3 contract. No mode file (or an expired one), no consult on these terms; an attended session consults via `skills/consult/SKILL.md` (its own attended semantics — no cap, no halts) or just asks the user.

...
- **Mode-gated.** Consults exist **only under an active mode** — a valid mode file per the step-3 contract. No mode file (or an expired one), no consult; an attended session just asks the user.

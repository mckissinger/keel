...
- **Consult the project's accumulated learnings first.** If the project keeps a durable learnings log (a `lessons/` dir, `decisions/`, the stack profile's **Q12 failure-signature table**, or other stack-profile gotchas), read the relevant ones — this exact defect class may already be solved. Don't re-derive a known fix.
...
3. **Root cause, not symptom.** Name the underlying cause and why it produces the symptom. A fix that makes the symptom disappear without explaining the cause is a patch, and it'll resurface. **When two or more root causes stay plausible and the evidence does not discriminate between them, that is a consultable judgment question** — dispatch it per `skills/consult/SKILL.md` (cited, never restated) rather than committing to the hypothesis you happen to be holding.
4. **Fix through the normal gate — debug owns diagnosis, not landing.** Route the fix by one rule: a fix with **no design/spec judgment** (the change is fully stated by the diagnosis — a wrong constant, an off-by-one, a missing guard) → a **`punch-list` item** (batched with its peers, one chore PR, one batch pin); a fix needing a **spec'd done-condition or any design decision** → **`spec-change`** (one milestone: branch → `implement-milestone` → `verify-milestone` → pin). This session's output is the diagnosis + regression test handed to whichever lane fits — never an ad-hoc patch to `main`, and never a third lane of its own.

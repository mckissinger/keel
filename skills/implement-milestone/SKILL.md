...
model: claude-opus-5
effort: high
allowed-tools: Bash(git checkout -b *), Bash(git branch *), Bash(git add *), Bash(git commit *), Bash(git fetch origin*)
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
...
1. **Orient first — before branching.** Derive the current state, don't assume it: read the `verified:` pins in `specs/milestones/`, `gh pr list`, and the recent `git log`; re-read *this* milestone's spec + `specs/stack-profile.md` from `main` (not a stale branch copy). The canonical state sweep is owned by the `status` skill — its derivation (pins, PRs, branches, worktrees) is the one to run here rather than an ad-hoc variant. **Entry preflight: run the profile's Q12 substrate health check first** (seconds — daemon responsive, ports owned by *this* project's stack, toolchain resolves, env file present or re-derived via the recorded command, env-var names asserted via the profile's recorded name-check command — never by reading the env file directly), **and the profile's stack-hygiene check where one is recorded**: foreign project stacks running on the machine are surfaced with their named teardown commands — offer teardown attended, record a run-note unattended, never tear down another project's stack yourself (Q12 — foreign stacks degrade suites invisibly; the 55-container scar); a red substrate is a **stop-point routed to the Q12 signature table's remedy** — never absorbed into the milestone, never debugged ad hoc mid-build. Then run the profile's **cheapest committed rung once** (the Q11 static/lint tier — a few seconds) to confirm a green baseline before you write a line. **A pre-existing red is a stop-point, not something to absorb** — surface it and route to `debug`; building on top of a broken baseline hides which failure your change caused. (Under batched/by-wave cadence the plan on `main` may have moved since it was authored — orienting is how you catch that.) **If this session runs in a worktree, name the untracked-state gap as a fact:** untracked files living in the primary checkout — `.env.local`, marker files — are absent here by construction; re-derive the env via the profile's recorded command and treat any absence as "not present in this worktree", never as evidence the project lacks the file (and never propose deleting a sibling worktree to resolve it).

2. **Branch first — before any code, and from the remote default, never local `main`.** `git fetch origin && git checkout -b <slug> origin/main` as the *first code step* (or off the parent milestone's branch when this one genuinely stacks). A local `main` behind the remote forks the milestone off stale code — the recorded scar: a wave cut from a `main` 17 commits behind drifted every branch in it, and a sibling session built without its parent milestone's merged code. Writing code on the wrong or stale branch is the most common recoverable mistake; fetching and branching first prevents both.
...
8. **Surface the choices you were unsure about — then hand off.** Before handing off, write **zero or more** uncertain-choice records — **one file per choice** — under `specs/uncertainties/<milestone-slug>/`, on this milestone's branch, committed with the milestone's code. `<milestone-slug>` is the **canonical spec-file stem**: `specs/milestones/<slug>.md` → `<slug>` (the same string `verify-milestone` reads, so the reader can never miss the writer's entries). Each file names **the choice made**, **the viable alternative(s) considered**, and **why it's uncertain** (what a reasonable reviewer might decide differently). File-per-entry, so parallel milestones never collide — the same discipline as `decisions/` / `deferrals/`.

   **The logging bar (log only if all three hold):** (1) you selected among **viable alternatives**, (2) the **spec did not dictate** the selection, and (3) a **reasonable reviewer could have chosen differently**. A spec-dictated choice, a single-obvious-path choice, or a routine style pick is **not** logged. The bar is high on purpose — the value is a *short* list of real judgment calls, not a diary. **A milestone with no genuine uncertainty writes no entries, and that is a clean handoff, not a gap.**

   *Worked calibration.* Log: the spec said "remove the item" without saying whether removal is a soft-hide (recoverable) or a hard delete, and you chose soft-hide — a viable alternative with real consequences exists, the spec was silent, and a reviewer could reasonably want the other. Don't log: you ordered two independent form fields email-then-name, or named an internal helper `formatCurrency` — no viable-alternative-with-consequences, no reviewer meaningfully differs.

   This is a **complement to verification, never a replacement**: it adds a human-facing signal at the handoff and **removes nothing** — every done-condition is still checked, the pin gate is unchanged. Then commit the milestone's code on its branch and hand to **`verify-milestone`** (a fresh session/subagent renders the surfaces, drives the actions, and — only on a clean verdict — writes the pin). You never write the pin, never commit to `main`, never merge.

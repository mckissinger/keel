...
**Under the full protection contract** (strict branches-up-to-date; `decisions/2026-08-01-required-checks-protection.md`), a wave of independent siblings is a **per-sibling cycle, not a fire-and-forget loop**: each landing makes every remaining PR out-of-date, and updating one onto the moved `main` pulls the landed sibling's squashed code into the pin's drift window — so the `verified-pin` context goes red **by design, not by error**. The written remedy is the cascade rule below in its cheapest form: update/rebase onto the new `main` (an already-verified sibling's disjoint content makes this conflict-only, code byte-identical), **re-pin with `scripts/repin.sh` under the CI-green rule** (`${CLAUDE_PLUGIN_ROOT}/references/milestones-and-verification.md` §5 — the diff old-pin→new-tip is empty outside plan paths, so the PR's re-fired required checks are the re-pin evidence; no local suite re-run), and let the contexts re-fire. The stacked-series choreography below needs no such cycle — its update-branch merges are content-identical, so drift stays plan-only there. The `babysit-prs` verb runs this per-sibling cycle as a background loop (watch → update → CI-green re-pin → drain under a standing authorization, or hold everything prepared for the user's taps), executing these rules unchanged.
...
- **Watching a wave follows the settle-only rule** (`${CLAUDE_PLUGIN_ROOT}/references/dispatch-and-monitoring.md`): report all-settled or first-failure, never per-check progress turns, and treat empty `gh` output as a failed read to re-check — never as "nothing left to merge" (the false ALL-MERGED scar).
...
- **Conflict-only rebase** (no behavioral diff): **re-pin with `scripts/repin.sh <milestone-spec> [note]`** — it rewrites the pin to the new code tip with a carry-forward clause, commits it plan-only, and asserts the postconditions. The green evidence stays your job (the script runs no tests), and which evidence depends on the diff: code byte-identical outside plan paths → the PR's green required checks at the new tip are the evidence (**the CI-green rule**, `${CLAUDE_PLUGIN_ROOT}/references/milestones-and-verification.md` §5 — no local re-run); conflict resolution touched code content → run the suite green locally first. Not a full re-verification either way.
...
3. It's a **conflict-only rebase** — content is identical (disjoint file ownership made the integration tree the same as `main`). Apply the cascade rule above unchanged: content-identical means the CI-green rule carries the suites (the PR's green checks at the rebased tip; no local full re-run) — **but re-run the milestone's own whole-repo terminal guard green locally** if it names one (that guard is the diamond's point, and its corpus just changed shape), then **re-pin with `scripts/repin.sh`** (carry-forward clause, plan-only commit; the script runs no tests). Not a re-verification.

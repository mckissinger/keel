...
**Interview the conversion delta, not the whole product.** What's the page's one job (signups? demos? installs?), who arrives, and at what **awareness stage** — the stage picks the copy framework (`references/conversion-craft.md` owns the mapping). Write the findings into the brief using `templates/marketing-brief.md` (product, audience, awareness stage, conversion goal, grain, brand/token source) and confirm it with the user before anything visual — a wrong brief poisons every movement downstream. When the product repo carries `specs/gtm/` (positioning authored by `gtm`), the brief consumes that committed positioning — audience, awareness, category — rather than re-interviewing it.

When the product repo carries a committed logo kit (`design/brand/`, authored by `logo`), Discover consumes it — the header mark, favicon, and `og:image` derive from the kit rather than being improvised.

**Pull the section-level pattern research — structure, not looks.** When a connected design-reference MCP's corpus includes marketing pages, a **pattern-level pull runs during Discover, default-on** (not a step a preference skips): section-level research into which sections comparable products' pages carry, how this category shapes its pricing and proof, where the actions sit. It is the marketing analog of `spec-feature`'s flow pull — run it per `references/reference-research.md` (the owning reference for this verb's reference research), using **`visual-reference.md`'s delegation mechanics only** — the pull and its distillation happen in a subagent that hands back compact text, never raw volume into this session — cited, not restated. Findings land as **structure** — in the brief and the section-system plan — **never as tokens or visual choices**; looks stay gated at Movement 2. **With no such MCP connected, proceed text-only and record that mode** — a paid source is never a hard dependency.

## Movement 2 — Explore

- **Post-app fork: an amplification round, not a from-scratch one.** The design language is already decided — do not re-litigate it. Build **2–3 throwaway variants of the committed design language at marketing spectacle**, differing mainly in **hero treatment** (the menu and its guardrails live in `references/hero-treatments.md`) and tone. One `templates/variant-spec.md` per variant keeps the divergence honest. When a design-reference MCP with a marketing-page corpus is connected, a **scoped looks-level pull** may inform **only these variant axes — hero treatment, section spectacle, and motion posture** — run per `references/reference-research.md` (which cites `visual-reference.md`'s mechanics, never restates them). Two guards bind it as explicit prohibitions: **the committed design language is never re-litigated**, and **the committed tokens are amplified, never replaced** — the pull strengthens the amplification round, it never opens a side door to a redesign. The references feed the 2–3 variants, which still converge into the marketing section system per the rules above, and **each variant spec records the reference URLs and extraction notes it drew from** (lineage).
- **Greenfield fork: a compressed from-scratch direction round** — the `app-design-directions` Phase 1–2 discipline at one-screen scale: distinct directions, real copy from the brief, anti-slop bar — **including its Phase 0.5 visual-reference pass** (there is no committed design language to protect, so the full reference-fed direction round applies).

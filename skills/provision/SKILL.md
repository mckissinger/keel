...
3. **Provision dev resources.** Where the host offers managed integrations (e.g. Vercel Marketplace via `vercel:bootstrap`) they auto-inject env vars; otherwise create dev-dedicated resources via CLI — a dev database project, test fixtures. Manual keys (e.g. a spend-capped AI-provider key) go into the host env store, then pull a local copy (e.g. `vercel env pull .env.local`). The host env store is the source of truth; `.env.local` is a derived copy.

4. **Seed the project allowlist from the contract.** Generate or update `.claude/settings.json` (committed) with allow rules for each contracted service's dev-loop commands (the entries below are examples of the pattern; the contract's services determine the actual set):

...
- runs one **worktree-isolated** subagent per group (make exactly the change, self-check, return a diff — no scope creep), each **dispatched with an explicit `model: claude-opus-4-8` arg at `low`/`medium` effort** — the build model at reduced effort for mechanical batch work, per `${CLAUDE_PLUGIN_ROOT}/references/model-routing.md` (the per-invocation dispatch arg, top of the resolution order),

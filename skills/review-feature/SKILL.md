...
3. **Judge with vision + the human.** A vision model looks first (catches "looks wrong" objectively); then present the side-by-side set to the user for the aesthetic + completeness call. Two distinct verdicts:
   ...
   - **Fidelity-to-intent** — does it read as the feature we composed? Not pixel parity, but: the structure, hierarchy, density, and signature element of the workbench composition. Drift toward generic is the thing to catch.

...
3. **Judge with vision + the human.** A vision model looks first (catches "looks wrong" objectively); then present the side-by-side set to the user for the aesthetic + completeness call. Four distinct verdicts:
   ...
   - **Fidelity-to-intent** — does it read as the feature we composed? Not pixel parity, but: the structure, hierarchy, density, and signature element of the workbench composition. Drift toward generic is the thing to catch. The **reference lineage recorded in the design decision file** (the chosen direction's reference screens — URLs + extraction notes) is consultable context for what the composition was converging toward — as is the **feature spec's sketch lineage** (a divergence sketch's own reference URLs + extraction notes), when one exists — supplementary, like the divergence sketch, never the reference itself.
   - **Hierarchy / density / expression** — does each screen carry actual visual hierarchy, or is it flat? The vision pass **grades every screen on hierarchy/density/expression *before* presenting to the user** — distinct type levels where semantic roles differ, a focal point among peer regions, differentiated action weights — so flatness is a named finding in the set, not something the human must notice unprompted. **An expression gap — any of the shared rules' §3 flatness signals (uniform type across a region with mixed semantic roles; adjacent equal-weight cards with no focal point; an action row of undifferentiated ghost buttons) — is an objective finding, never waved through as taste.**
...
**When the gate passes** — no findings, or the refinement milestone closed and re-reviewed — **record the closure in the feature spec's Lifecycle section** (`specs/features/<feature>.md`): a fact with evidence (the date + what was reviewed), opened as a **plan-only PR** — the same flow as `land-feature`'s reconciliation step: plan paths only, pin-exempt (§5), landing under the standing merge authority (the user's tap, or `--auto` under a valid marker/mode; the protection contract forbids direct pushes to `main`). Plan-only by construction, it carries no code and needs no pin. That entry is the durable answer to "did we do feature review?".

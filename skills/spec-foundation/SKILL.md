...
3. **This skill writes the contract; `provision` executes it.** After this skill stops, the kickoff sitting hands to the design-system gate (`app-design-directions`, when the deliverable has a UI) and only then to `provision` (dispatched by `kickoff`) — which runs logins, dev-resource creation, allowlist seeding, and the dry-run while the user is present; the sitting ends on its green preflight.

## Stack profile

Derive **`specs/stack-profile.md`** — the project's answers to the verification interface (`${CLAUDE_PLUGIN_ROOT}/references/profile-interface.md`): what a surface is, how to activate one and assert it's healthy, how to drive an action and assert its effect, how to seed no-human test state, the dev vs production build, the schema-versioning scheme, **whether it has a UI**, the deployed surface, **the test-tier ladder (Q11)** — seeded from `${CLAUDE_PLUGIN_ROOT}/references/feedback-ladders.md` where an entry exists (priors, currency-checked by the research step, never adopted blind) — and **the local substrate contract (Q12)**: its structural parts (the shared singletons, the canonical invocation path) derived here; its provision-finalized parts (ports/identity, env re-derivation command, env name-check command, signature-table seeding, duration budgets, the proven-green health check) recorded as "finalized at provision", never invented at derivation. **The parallel-session isolation contract (Q13)** is recorded in the same pass, finalized-at-provision like Q12's provision-owned parts — derivation writes its default, "not provided — runtime-proof stays serial"; whether the stack opts in is `provision`'s question, never derivation's invention. **The scaffold-gap rule: if the scaffold doesn't ship a runner for a declared tier, kickoff adds it** — common web scaffolds ship no test runner at all, and a ladder first stood up mid-feature is the failure this rule exists to prevent (a keel run hit it live). Derive it from **reading the project** (dependency manifest, framework/build/test config) + **web research bounded by those questions** — keel ships no profile and no library; each project answers for its own stack. Propose it in the interview's synthesis and confirm it at the confirm-before-author gate. It is a durable spec artifact: `verify-milestone`/`review-feature` dispatch the runtime-proof through it, `provision` reads its shared-singleton answer, and its **has-UI?** answer gates the design track + the fidelity done-condition (a derived profile starts ~80% and hardens via `lessons/` as gotchas surface). The Q11 derivation also checks any existing test config for §8 rule 1's smell (`${CLAUDE_PLUGIN_ROOT}/references/milestones-and-verification.md` §8) — a suite serialized to paper over cross-test contamination — and surfaces it at derivation time rather than absorbing it (a deliberate serialize-or-isolate tier per the stack's ladder prior is not the smell).
...
- **The run preflight** (authorization-completeness): "Before a milestone run, confirm every milestone in scope can complete autonomously. (1) Services: one cheap read-only command per service where available, bare env-var-name existence otherwise via the profile's recorded env name-check command — never read values, never read the env file directly. (2) Spend/real-resource gates: any done-condition needing live spend must be pre-authorized (spend-capped key + allowlisted command), not gated per-use. (3) Deferrals: read `specs/deferrals/` — drain any open item whose closing condition falls inside this run's scope. Surface anything missing/dead/unauthorized in one batch before starting; if a gate can't be pre-authorized, treat that milestone as a stop point."
...
- **The build start protocol** (orient before building): "Before writing code for a milestone, orient: derive state (the `verified:` pins + `gh pr list` + recent `git log`), re-read that milestone's spec + `specs/stack-profile.md` from `main` (the plan may have moved under batched/by-wave cadence), run the profile's Q12 substrate health check (a red substrate is a stop-point routed to its signature-table remedy, never built on), and run the cheapest committed rung once to confirm a green baseline. A pre-existing red is a stop-point — route it to `debug`, never build on top of it. Then branch."
- **The GitHub process:**
  ...
  - **A conflict-forced rebase re-pins; it doesn't re-verify from scratch.** If a rebase changed only conflict resolutions with no behavioral diff, **re-pin with `scripts/repin.sh <milestone-spec> [note]`** — it rewrites the pin to the new code tip with a carry-forward clause and commits it plan-only; the green evidence stays the caller's job (the script runs no tests), and which evidence applies is `${CLAUDE_PLUGIN_ROOT}/references/milestones-and-verification.md` §5's call: code byte-identical outside plan paths → the PR's green required checks (the CI-green rule, no local re-run); conflict-resolved content → a local green run first. A rebase that changed behavior is a new state and must be re-verified.
  ...
  - **The user reviews and merges; agents open PRs and merge only on their explicit per-merge instruction.** Be precise about server-enforced vs behavioral: branch protection (once provision's preflight verifies it's *live*) blocks direct pushes/force-pushes/deletion of `main` — but does not by itself stop the shared agent token from *merging*. So "never commit to main" is a server guarantee; "only the user merges" is a behavioral rule. To make it server-true: a separate account without merge rights, or CODEOWNERS. **Merge-authority/permission changes are expressed only in PROJECT-level settings** (the repo's committed `.claude/settings.json` and hooks), never user-global — e.g. moving `gh pr merge` out of an ask list globally would grant merge authority to every project at once. A session asked to "merge as I go" routes to the attended auto-merge toggle (`keel:auto-merge`) or an active auto posture, never a global settings edit.
...
Under an active `keel:auto` mode (per `decisions/2026-07-05-autonomy-modes-v2.md` + `decisions/2026-07-genesis-envelope.md`) — reached at kickoff only via the **genesis** posture (`skills/auto/SKILL.md`), which seeds this skill from an approved product skeleton — spec-foundation's attended gates become **ledgered defaults**: the app-skeleton interview/confirm gates take the approved skeleton as their recorded input, and the synthesis plus the artifact each gate would have shown are written to the run ledger per `keel:auto`'s ledger contract, and the build proceeds without stopping. **Every structural rule is unchanged** — the repo setup below, the CI-gate wiring (`verified-pin` + `plan-lint` + branch protection), and the verified-pin gate all hold exactly as attended. This is additive: an active mode changes only *where the human judgment sat* (already spent at the genesis approval gate), never which gates the code must pass. Outside a mode, the attended kickoff holds exactly as written.

## Repo setup

If the project isn't a git repo yet, step zero is `git init` + GitHub remote, with minimal CI — **the CI job list is the profile's declared ladder (Q11: static / unit / component-render / e2e as declared), not a hardcoded trio** — plus **the `verified-pin` job** and **the `plan-lint` job** — copy keel's shipped `${CLAUDE_PLUGIN_ROOT}/scripts/check-verified-pin.sh`, `${CLAUDE_PLUGIN_ROOT}/scripts/check-plan.sh`, and `${CLAUDE_PLUGIN_ROOT}/scripts/repin.sh` into the project's `scripts/` and wire the two check scripts as CI jobs (`repin.sh` is the canonical re-pin mechanizer the rebase re-pin rule invokes — copied like the gates, not wired as a job; copy them, don't re-author them; their behavior is specified in `${CLAUDE_PLUGIN_ROOT}/references/milestones-and-verification.md` §5), and wire the ladder's suite-grade jobs with the plan-only early-exit via the copied classifier (`check-verified-pin.sh --plan-only-check`), per the Q11 fast-path rule in `${CLAUDE_PLUGIN_ROOT}/references/profile-interface.md` — **and write the sync manifest in the same commit**: `scripts/KEEL-SYNC`, first line `keel <installed plugin version>`, then one copied-script filename per line; every later copy or re-sync of a shipped script updates it, so any session can compare it against the installed keel version and remedy drift as **one batch sync chore PR** (re-copy every listed script) instead of discovering stale scripts one failure at a time — **and branch protection on `main`** (require a PR, block direct pushes, required status checks with **require-branches-up-to-date** — the up-to-date requirement is load-bearing for the pin gate: a moved base must re-enter the drift window before merge, forcing the branch to absorb it where the gate then demands re-verification of the combined state). Branch protection is what makes "the agent never commits to main / nothing autonomous crosses go-live" *server-enforced* rather than behavioral. **Two jobs is the whole kickoff tier — the third required check the auto-preflight asserts (`security-review`) belongs to the autonomy tier and is wired at auto-entry at the latest** (genesis wires it at bootstrap; `auto:feature`/`auto:run` on a standing project wire it as preflight remediation; wiring it at kickoff is also valid — keel's own repo does — in which case its context joins the required set the moment the job exists, never before), per `${CLAUDE_PLUGIN_ROOT}/references/template-contract.md` tier 1, which records its default implementation **and the proven concrete recipe** (the named-job shapes, the SHA-pinned review action with least-privilege permissions, and the **two protection shapes** — wire the kickoff-tier shape verbatim: required contexts are only the checks the kickoff wires, so a fresh repo is never wedged behind a context no job reports). The `specs/` folder lands as the first PR — the spec deserves its own review moment before any code. Foundation-window PRs (this first PR, the workbench, the scaffold) pass the gate via its **bootstrap window**: until the first milestone or chore spec exists — judged at both the base branch's tip and HEAD — the gate exempts PRs with an explicit bootstrap-window message, and it arms itself permanently the moment the first one lands.

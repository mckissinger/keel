...
description: Stand up a project's FOUNDATIONAL specs/ once at kickoff — the product skeleton (a feature backlog, not a milestone list), the architecture + data model + environment contract, the decisions/deferrals conventions, and the project CLAUDE.md with the GitHub/CI/verified-pin process. Use at the start of a new project after the app-skeleton interview, before the design system. Per-feature milestones are authored later, one feature at a time, by the spec-feature skill — NOT here.
---
...
2. **Test-mode credentials only until launch.** Dev gets its own Supabase project, Stripe test keys, a spend-capped Anthropic key. Record "live keys never enter this environment" in out-of-scope so violations are detectable.

   - **Deploy model: continuous deployment, single gated go-live.** `main` auto-deploys to Vercel production; every PR gets a preview — both **test-mode env** pre-launch, so "production" before launch is the prod build on real infra with no real users (harmless). **Previews are the verification surface.** "Launch" is the one discrete, **attended** event where live keys replace test keys, the real domain is pointed, and prod DB migrations are applied — the home of the `deferrals/` real-resource checkpoints. Nothing in an autonomous run crosses it: explicit deploy/resource/live-mode commands are never allowlisted, and a push to `main` (which auto-deploys) is blocked by **branch protection on `main`** (see Repo setup) plus the standing "never commit to main" rule.

3. **This skill writes the contract; `provision` executes it.** End the kickoff sitting by invoking `provision` — it runs logins, dev-resource creation, allowlist seeding, and the dry-run while Michael is present; the sitting ends on its green preflight.
...
- **The GitHub process:**
  ...
  - **Migration files use the framework's own timestamp scheme** (`supabase migration new <name>` → full UTC-timestamp version), never a hand-incremented counter (which forces parallel branches to coordinate a global next-number and collide at merge). Forward-only: existing counter-named files stay. Timestamps remove the *file-name* collision, not the *shared-schema-object* collision — when several sibling branches each need the SAME new DDL object on a shared parent table, make that statement **idempotent with an identical object name** (`do $$ begin if not exists (…) then <alter> end if end $$;`) so the earliest-timestamped migration creates it and the rest no-op (cre-launch's M26/M28/M24 each needed the same `contacts(id,org_id)` constraint).
  ...
  - **A wave isn't done until it's green on `main` together.** Each milestone is verified on its own branch; nothing on a branch proves the merged result (combined migrations in order, the coverage harness seeing every new tenant table, siblings coexisting). After a wave's PRs merge, run one **consolidated check on `main`**: fresh DB → all migrations → full suite (+ the consolidated first-run walk for UI waves). This is the only gate that catches cross-sibling integration bugs.
  ...
  - Merge execution: before each stacked merge confirm the PR's base is `main` (`gh pr view <n> --json baseRefName`). Never merge with checks pending or red. Merging requires Michael's per-merge approval (the `gh pr merge` ask rule). After each merge+delete, verify the next PR retargeted and is still OPEN (GitHub's auto-retarget races branch deletion and can CLOSE the dependent — recover with `gh pr edit <n> --base main` then `gh pr reopen <n>`).

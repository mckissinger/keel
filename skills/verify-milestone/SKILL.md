...
4. **If the milestone adds/changes a surface (Q1–Q2 of the stack profile — a route/screen/endpoint/command) or a state-changing action, the `[runtime]` walk is part of the gate — run it here, in addition to the named verifier/workflow.** This session is the path that *can* (it has the profile's activation driver + the local services to itself; the parallel `verify-all-milestones` sweep cannot). Per the project's `specs/stack-profile.md`: stand up the local services, seed a **no-human authenticated/test state** (Q5), then for each new/changed surface — activate it and assert it's healthy (Q3: no error boundary / 2xx / clean exit, no console/network errors) and drive each new action and assert its **effect** (Q4). Run against **both the dev/fast build and the production build** (Q6); for an AI/live milestone, the **capped-key live variant** within the pre-authorized envelope (Q7). This is the check that catches what unit tests can't — the test-runtime-vs-production and dev-vs-prod-build divergences. See `${CLAUDE_PLUGIN_ROOT}/references/milestones-and-verification.md` §3 and `${CLAUDE_PLUGIN_ROOT}/references/profile-interface.md`.

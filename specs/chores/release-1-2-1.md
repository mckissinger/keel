...
verified: clean at 1c02c85, 2026-07-04, via fresh-context verifier subagent — branch changes exactly two files (`plugin.json` + this chore spec); the `plugin.json` diff is exactly the `"hooks": "./hooks/hooks.json"` line removed plus `version` `1.2.0`→`1.2.1` and no other field; the result is valid JSON; `hooks/hooks.json` is unchanged vs main and still carries the complete global registration (SessionStart→session-bootstrap.sh, PreToolUse Bash→merge-guard.sh), so no hook is lost; the skill-scoped `guard-branch-rules.sh` registers via SKILL.md (implement-feature :10 / implement-milestone :11), not `hooks/hooks.json`, so it is unaffected; `claude plugin validate --strict .` passed, check-neutral + check-plan green, both guard self-tests green (merge-guard 81, guard-branch-rules 30); 1.2.0→1.2.1 is a valid patch. The hooks-still-register-after-removal property is the disclosed [runtime] check closed by the post-merge install+restart dogfood, correctly out of branch scope. (evidence: verifier report in PR #75)

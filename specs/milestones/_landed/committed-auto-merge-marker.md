...
verified: clean at e44d1f5, 2026-08-02, via a fresh-context verifier subagent against this file — every `[auto]` condition checked against the real code. Root of trust is the SERVER default branch: `read_committed_marker` (merge-guard.sh:426-466) resolves the default branch from `gh repo view --json defaultBranchRef` in GIT_CTX (L453, NOT `origin/HEAD`), charset-guards the branch name (L455), does a MANDATORY fail-closed fetch (L456, `|| return 0`), reads `refs/remotes/origin/<db>:.claude/keel-auto-merge.json` via `git show` parsed as data (L457-461), and requires `scope=="project"` + non-empty `created`/`invoker` (L462-463) — every defect fails closed, no TTL. `is_plan_path` (check-verified-pin.sh:62) is EXACTLY one literal case arm `.claude/keel-auto-merge.json) return 0`, not a `.claude/*` wildcard; the test pair confirms marker-only PR exempt (check-verified-pin.test.sh:350) and marker+code PR still fails (L359). `decide()`'s committed row (merge-guard.sh:821-830) has the five-way guard (MODE_ACTIVE=0 && ATTENDED_ACTIVE=0 && COMMITTED_ACTIVE=1 && AUTO_MERGE=1 && SHAPE==gh-pr-merge), sits inside the gate-pass block, binds `d_auto="allow"` (no bare literal), precedence mode>attended>committed. `pr_touches_marker` (merge-guard.sh:749-773) is checked first inside the gate-pass block (L802, before every allow row), with a mandatory base+head fetch (L767-768), `-c diff.relative=false --no-renames` and `base...head` (L769-770), failing closed to ask on every path. guard-branch-rules.sh has the mandatory-fetch committed reader (L463-478) and the defer row (L563-569), never emits `allow` (exit-code only), and merge-guard.sh is the globally-wired backstop (hooks.json PreToolUse/Bash). The four security fixes were independently confirmed real against `git show 30adfe1`: old code used best-effort fetch + local `origin/$DEFAULT_BRANCH` (findings 1/2), no head fetch (finding 3), and no `diff.relative=false`/`--no-renames` (finding 4); the five regression cases in merge-guard.test.sh (L754, 771, 789, 807, 826) all assert `ask`. The previously-failing committed-reader jq/python3 parity is now exercised: attended-marker-parity.test.sh arms the committed marker on the server default branch (L162-170, marker leaves the working tree) and runs valid + three fail-closed cases under both jq and a jq-omitted python3 SANDBOX (L40 omits jq) across both guards. All suites green (attended-marker-parity 20, merge-guard 140, guard-branch-rules 79, check-verified-pin 38, check-auto-preflight 29), all lints green (check-neutral, check-skill-frontmatter, check-skill-anchors, check-plan), `claude plugin validate --strict .` passed, and `git diff --stat ad859b0..HEAD` is confined to the 3 guards (+ tests), the shared parity test, the decisions entry, the uncertainties note, and this spec — no production surface outside the three guards. (evidence: verifier report in PR)

...
verified: clean at 91c7b6c, 2026-07-04, via verifier subagent against this file (every [auto] + the [runtime] deterministic core confirmed against source with file:line evidence; decision matrices trusted from the committed self-tests, not re-derived) + /security-review pre-pin — all committed suites green (merge-guard 81, guard-branch-rules 30, attended-marker-parity 8 [5× convergent], session-bootstrap 30, check-neutral/check-plan/check-skill-frontmatter/check-verified-pin), `claude plugin validate --strict .` pass, all three new files git-tracked and none gitignored. Security review clean: the sole candidate — an agent-forgeable untracked marker — is the documented, accepted residual of the same class the autonomy mode file already carries (recorded in specs/deferrals/mode-file-binding-ttl.md and this spec's contract §, backstopped by delegation-to-required-checks since the marker unlocks only the --auto handoff), not an unintended vuln; no eval/interpolation, detect_strict_auto whitelist resists chain/quote/alias/non---auto bypasses, autonomy precedence fails safe, reader parity holds. The [attended] end-to-end no-prompt bullet remains the human's dogfooding confirmation — its machine-checkable ground truth (the emitted decision JSON) is covered by merge-guard.test.sh. (evidence: verifier + security reports in PR)

...
verified: clean at 4aa86e8, 2026-07-05, via a fresh-context verifier subagent against this file — all three artifacts present with every enumerated element: the decisions entry (a)–(f) (bounded Phase 2 envelope; spend-caps-as-authorization, names-only keys "never reads, echoes, logs, or prints"; never-auto list quoted verbatim and unchanged; rejection creates nothing; the forgeability residual naming specs/deferrals/mode-file-binding-ttl.md + server-side required checks as backstop; the §(a) enumeration extended to level "genesis" by reference with §(e) unchanged); the two-tier template contract (tier-1 gates mapping 1:1 onto check-auto-preflight.sh (a)–(d), (d) = the allow_auto_merge assertion confirmed present in the script at line 180; tier-2 prune rule as a checkable condition; the strict-gates/loose-stack asymmetry stated, unused key never an error); the deferral's buy/why/reopen (first successful genesis dogfood run). By-reference discipline VERIFIED: `git diff main` empty for both decisions/2026-07-autonomy-modes.md and decisions/2026-07-01-model-capability-ledger.md; diffstat shows only the three new doc files. Adversarial invariant attack found NO sentence weakening the verified-pin gate, human merge authority, or the never-auto list. Gates + all 10 self-test suites green (merge-guard 90, guard-branch-rules 35, session-bootstrap 33, attended-marker-parity 12, check-auto-preflight 20, check-neutral 17, check-skill-frontmatter 12, check-plan 18, check-skill-anchors 11, check-verified-pin 19) + `claude plugin validate --strict`. Prose-only milestone — no /security-review (that gate is m3). (evidence: verifier report in PR)

...
- **Kickoff→run seam = charter seeding, prose only.** Today `keel:auto run`'s entry audit
  ...
  build order, resolved decisions — where the run's entry audit looks first, and offers the
  `keel:auto run` invocation (offer only: the human invocation stays the authorization; kickoff
  never starts a mode itself).
...
- [auto] `skills/kickoff/SKILL.md`: the final step (post-preflight) writes the kickoff synthesis
  to the project's run-ledger location as a charter seed and offers — never invokes — the
  `keel:auto run` continuation, naming the human invocation as the authorization.
- [auto] `skills/auto/SKILL.md`: the entry-state audit (step 1) names the kickoff charter seed
  as its first input when present, falling back to today's from-scratch audit unchanged.
...
- [auto] `scripts/check-neutral.sh` and `scripts/check-skill-frontmatter.sh` pass on every
  touched skill/agent file; no guard script, hook, or `scripts/` behavior changes in this
  milestone.

### Behavioral completeness
- [auto] Grep-assertable text anchors exist for each prose change (committed self-test or lint
  additions asserting: verify-milestone names the stack profile in its dispatch section;
  verifier.md names `stack-profile.md` as runtime ground truth; kickoff names the charter seed +
  offer-not-invoke; auto step 1 names the charter seed) — added to the existing skill-eval /
  lint tier so regressions fail CI, per the fixture pattern `scripts/check-skill-frontmatter.sh`
  established.
- [auto] `claude plugin validate --strict .` passes; every pre-existing self-test suite still
  passes (no script behavior changed, so all suites run against unchanged scripts).

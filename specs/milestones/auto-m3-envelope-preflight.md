...
verified: clean at 32baeb7, 2026-07-02, via fresh-context verifier subagent; /security-review PROCEED (zero findings — unquoted case-glob, indirect expansion, heredoc, and every output path cleared with empirical probes; no value-leak channel) — envelope prose a–e confirmed in place inside the unattended-run posture section with line evidence, generated-CLAUDE.md preflight line adjacent to the posture line, check-auto-preflight.sh traced line-by-line (inventory glob dry-run, gh api required-checks assertion from config block, ${!name+x} name-only env resolution, fail-closed on missing gh/jq/inputs), 13/13 new suite green incl. sentinel value-leak assertions, one-owner sweep clean, diff touches only owned files + the permitted check-neutral scan-target line, check-neutral PASS + check-plan PASS + all suites green (13/17/18/17/16/29/14) (evidence: verifier + security reports in PR)

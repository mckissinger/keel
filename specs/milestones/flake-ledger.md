...
verified: clean at e40381b, 2026-07-21, via fresh-context verifier subagent against this spec's done-conditions — all conditions evidenced with file:line: `specs/flakes/README.md` establishes the file-per-flake convention (six shape fields, drained by a batch chore, sibling to `specs/deferrals/`, empty with no backfill, no after-N-repeats auto-promotion); `references/milestones-and-verification.md` §8 gains rule 9 (measured-rate + file-per-flake + batch-chore triad, citing verify-milestone's run-to-convergence rule by home, not restated) with rules 1–8 byte-unchanged and no `## 9` section created; `skills/status/SKILL.md` surfaces open flakes in the Sources sweep + output-contract item 5 (highest hit count first) with the deferrals sweep unchanged and no write added; anchor file pins both load-bearing sentences (file-per-feature). Diff is exactly the four spec'd files — no leakage from the sibling suite-execution-doctrine milestone. No `[runtime]` conditions (keel is no-UI) → no runtime walk; prose-only → no `/security-review`. check-neutral / check-plan / check-skill-frontmatter / check-skill-anchors (54 anchors) green, all 11 `scripts/*.test.sh` suites green, `claude plugin validate --strict` PASS. (evidence: verifier report in PR)

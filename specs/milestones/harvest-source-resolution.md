...
verified: clean at 7810b30, 2026-07-20, via fresh-context verifier subagent against this spec, executing the enumeration + extraction commands and `bash scripts/check-skill-anchors.sh` itself. **Enumeration discriminates**: 6 directories yield three distinct counts under the mtime-inclusive predicate (e.g. `-Users-michaelkissinger-Dev-Projects-keel` A=2 top-level-filtered < B=37 recursive-filtered, A=2 < C=10 top-level-unfiltered); the committed command emits count 2 for that row. **Watermark read, not hardcoded**: the committed block reads `2026-07-18` from the cursor's `**Watermark:**` line and emits 7 rows; with `HARVEST_WATERMARK` set to 2026-06-18 (one day before the oldest top-level jsonl, 2026-06-19) it emits 39 — different counts; the command block (lines 50–58) matches no `[0-9]{4}-[0-9]{2}-[0-9]{2}` literal and `2026-07-18` appears nowhere in SKILL.md. **Sweep from filesystem**: command reads only the Watermark line, no table-parse construct, output >1 row. **Extraction** against `d89b3abb-…jsonl`: 26 `.origin.kind==human` + 2 non-empty `<command-args>` = 28 lines, ZERO `<command-name>`/`<command-message>`/`<local-command`/`Base directory for this skill` leaks. **newest-date**: keel row emits 2026-07-20 = independently-computed newest qualifying top-level mtime. **Anchors**: `check-skill-anchors.sh` green (47 anchors); all 7 retirement anchors present at merge-base e61c91d (MB≥1) and absent now (NOW=0); prohibition anchor `2026-07-18` absent from SKILL.md. **Grant closure**: every external executable in the committed blocks (grep, sed, find, wc, tr, stat, sort, head, basename, jq) has a matching `Bash(<cmd> *)` grant. **No weakening**: secret rule, diff-against-HEAD, writes-only-digest-and-cursor, "No cursor in the target repo = ask", `disable-model-invocation: true`, two-scope structure, fan-out-via-read-only-subagents all retained. **Standing suites** green: check-plan (64 milestones/28 chores), check-neutral, check-skill-frontmatter (29 skills), check-skill-anchors, `claude plugin validate --strict .`. Deferral `specs/deferrals/harvest-retro-scope.md` present, status open, closing on the first genuine retro run; cursor `through` column and `+N` rows removed, duplicate dirs collapsed; review `2026-07-18-harvest.md` caveat upgraded to a location citation. No runtime walk — the executed commands are the runtime proof, per the verification line. (evidence in PR #159)

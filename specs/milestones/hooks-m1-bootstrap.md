...
verified: clean at 3dbd3cd, 2026-07-01, via fresh-context verifier subagent — all [auto] conditions pass: hooks.json valid (top-level "hooks" key, single alternation matcher startup|resume|clear|compact, quoted ${CLAUDE_PLUGIN_ROOT}) with the schema independently confirmed against code.claude.com/docs (compact is a documented SessionStart matcher and THE re-injection path — PostCompact cannot inject; |-separated matcher lists and plain-stdout-as-context on exit 0 are both documented); plugin.json registers ./hooks/hooks.json; session-bootstrap.sh is 100755 in git, set-euo-pipefail-safe, probes all three markers, silent exit-0 with no marker, and emits a 167-word bootstrap faithful to README's grain ladder with all five standing invariants; self-tests run fresh in the verify worktree (session-bootstrap 11/11 incl. the word bound + compact grep, check-neutral 15/15 incl. the new hook-file + bootstrap hardcode cases, check-neutral repo scan PASS, check-verified-pin 17/17) and `claude plugin validate --strict .` passes; diff touches only the seven named files, no stray staging dir in repo or worktree. Disclosed interpretations accepted: single alternation matcher and plain-stdout are doc-canonical; self-contained script satisfies the path-churn intent; CI wiring of session-bootstrap.test.sh deferred — carry it into m2/m3, not dropped. The [runtime] activation check rolls to the m3 composition walk per this section (deferred to m3, NOT closed here). (evidence: verifier report in PR)

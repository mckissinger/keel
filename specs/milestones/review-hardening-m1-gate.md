...
verified: clean at ee5080f, 2026-07-06, via fresh-context verifier subagent (initial at 0f7648c found one discrepancy + a delta re-verify of the final SHA: CLEAN) — merge-base precondition fails closed before the three-dot diff (orphan-history repro fixture FAILS naming both refs + shallow-clone/unrelated-histories cause + fetch-depth: 0 fix; live [runtime] disconnected-shallow-tips fixture: --depth 1 tips → FAIL closed, post-full-fetch → proceeds to ordinary pin rules); plan-exemption carve-out three-way verified (stack-profile-only and inventory-only PRs require a pin, post-pin edit is drift, pinned change passes, decisions/deferrals-only stay exempt); guard-branch-rules reader-less degrade CLOSED via raw substring scan (verifier's tab-escape/ -escape bypasses of the initial \b-anchored scan were FIXED pre-pin at ee5080f and pinned as a suite row; benign input exits 0; leanbin-py/leanbin-jq reader-present rows byte-for-byte, incl. the fresh-marker defer row); merge-guard.sh byte-identical to main (git diff empty) while its suite pins the accepted sh -c / bash -lc / eval / piped-xargs bypasses as silent-unclassified (R1 has no gate, so classification would emit ask — the assertions pin, not echo). No-eval / quoted-data review of every changed line: RAW only ever printf'd into grep, all expansions quoted, no interpolated command text; check-neutral PASS. All 10 suites green (check-verified-pin 27, guard-branch-rules 54, merge-guard 101) + `claude plugin validate --strict`. **Pre-pin /security-review: NO findings** (>=8/10 confidence bar) — merge-base gate unreachable-bypass probed (no ref pair reaches "no changes — pass" without a real merge base), carve-out is strictly tightening against git-normalized paths, fallback has no injection/eval surface. Verifier's judgment-flag: a case-variant `specs/Stack-Profile.md` stays plan-exempt per the done-condition's letter ("all other specs/** paths keep today's classification") — a live smuggle vector only on case-insensitive checkouts; surfaced in the PR for the acceptor. (evidence: verifier + security reports in PR)

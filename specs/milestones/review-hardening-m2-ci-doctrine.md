...
verified: clean at 87c9345, 2026-07-06, via fresh-context verifier subagent — CI step event coverage verified against workflow semantics (pull_request-only `if` skips on push so the empty base_ref is never evaluated; base_ref expands into env:, not the run: body, so no shell-injection path; fetch-depth 0 satisfies m1's merge-base precondition); all four prose surfaces converge on the two-part-control claim checked against the ACTUAL shipped mechanism (gate header incl. the explicit does-NOT-prove-verification line + squash/SHA nuance + require-branches-up-to-date dependency; merge-guard accepted-limits para gains eval + names branch protection + required checks as authority; branch-guard header documents the same limits + the m1 fail-closed degrade; both banner variants name both halves) — with `git diff main` confirmed COMMENT-ONLY on all three guard scripts (every changed line is a # line; m1's logic untouched); the three header-doc tripwires verified fail-on-revert empirically (each grep set run against main's pre-m2 files fails). spec-foundation + adopt carry require-branches-up-to-date with the why; squash/merge-commit doctrine unchanged. decisions/2026-07-05-pin-gate-honesty.md amends the ledger by reference (ledger diff empty) in confound-honest style; specs/deferrals/post-merge-drift-check.md carries rationale + revisit trigger. All 10 suites green (session-bootstrap 36 w/ reworded fixtures, merge-guard 102, guard-branch-rules 55), check-skill-anchors 14 anchors PASS (no deletions in diff), check-neutral PASS, `claude plugin validate --strict` PASS. **[runtime]**: the exact CI command line (BASE_REF=origin/main bash scripts/check-verified-pin.sh HEAD) run live from the full-depth checkout returned the correct pre-pin verdict (FAIL, unpinned code PR, rc=1) — the step's first in-CI execution is observable on this PR post-pin. Verifier observation (pre-existing, outside m2's surfaces): skills/verify-milestone/SKILL.md:61 still says plan paths = specs/**+design/** and an unscoped verified==merged line — flagged for a later lane. (evidence: verifier report in PR)

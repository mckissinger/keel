...
verified: clean at d2ffc4e, 2026-07-03, via fresh-context verifier subagent against this file — all 6 [auto] conditions PASS with file:line evidence: the lint is 100755 in git + dirname-relative (`check-skill-frontmatter.sh:36`), field-presence-only parsed strictly from the frontmatter block (name==dir, description non-empty, when_to_use non-empty) with NO semantic/"NOT"/workflow-summary inspection (negative grep clean); self-test 12/12 including the workflow-summary-passes anti-overreach case (exit code + offending field on stderr asserted, not just tallied); `check-skill-frontmatter.sh` PASS over all 16 skills with ZERO skill-file edits (no `skills/` path in the diff); `.github/workflows/ci.yml` adds exactly the two lint steps mirroring check-plan and alters no other step; full diff vs main is exactly the three intended files; check-neutral + check-plan still green. No [runtime]/surface → no runtime walk; no hard invariant → no /security-review. (evidence: verifier report in PR)

...
verified: clean at 20cf588, 2026-07-21, via fresh-context verifier subagent against this spec's done-conditions — all conditions evidenced with file:line: the Q6 build/server-freshness scar (freshly-built-for-the-commit, run-started-server-never-adopted, busy-port-fails-loud — all three, stack-neutral, Q6's both-builds rule intact) and the Q11 CI-topology scar (built-once-shared, deps cached, sharding multiplies fixed setup) in `references/profile-interface.md`; a new `## 9. Suite execution` section in `references/milestones-and-verification.md` with §9.1 (full-suite / no-filter + inner-loop exemption, not touching §3's coverage gate or §5's pin rules) and §9.2 (foreground-with-timeout keyed to the Monitor/poll boundary, deferring the 2×-budget kill-bound to Q12); the Q12 unconditional-backgrounding clause retired (conditional now, foreground path cites §9.2) and confirmed absent via a negative anchor (`grep -cF` = 0); file-qualified §9.1 dispatch pointers in `verify-milestone` step 4 and `implement-feature` step 2. Built off the updated main: flake-ledger's §8 rule-9 list item and provision-hardening's Q12 port rule are both intact and coexist coherently (a `## 9` section vs a "rule 9" list item — distinct, no collision); §1–§8 not renumbered; pre-existing bare `§9` citations still resolve to `growth-operations.md` §9. No `[runtime]` conditions (keel is no-UI) → no runtime walk; prose-only → no `/security-review`. check-neutral / check-plan / check-skill-frontmatter / check-skill-anchors (62 anchors) green, all 11 `scripts/*.test.sh` suites green, `claude plugin validate --strict` PASS. (evidence: verifier report in PR)
